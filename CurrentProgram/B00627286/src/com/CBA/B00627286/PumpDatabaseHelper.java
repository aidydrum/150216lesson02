...
public class PumpDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void addProduct2db(VehicleDetails item) {  // this is called in PumpFuelGaugesActivity.java
    ...
    public ArrayList<VehicleDetails> get_All_User_Inputs() {  // clear the list
        database_List.clear();
        ...
        //cursor.moveToFirst();
        if (cursor.getCount() != 0) {  // position the cursor at index position Zero
            if (cursor.moveToNext()) {
                do {  // VehicleDetails objects
    ...
    public ArrayList<VehicleDetails> Result_Litres_Used_Main_Vehicle() {  // clear the calculated list
        // initialize the ArrayList of Vehicle Details
    ...
    public ArrayList<VehicleDetails> Result_Litres_Used_Secondary() {  // clear the calculated list
        ...
        if (recordCheck != 0) {
            //if (cursor.moveToPosition(cursorMover))
            if (cursor.moveToFirst()) {  // Cursor is set to start at index pos 0
                VehicleDetails currentItem = new VehicleDetails(); // look at current index
                VehicleDetails nextItem = new VehicleDetails();    // look to next index
    ...
    public ArrayList<VehicleDetails> milesTravelled_Main_Vehicle() {// clear the calculated list
    ...
    public ArrayList<VehicleDetails> milesTravelled_Second_Vehicle() {  // clear the calculated list
    ...
    // get cost per mile
    public ArrayList<VehicleDetails> Cost_Per_Mile_Main_Vehicle() {  // clear the calculated list
        ...
        if (recordCheck != 0) {
            ...
            if (cursor.moveToFirst()) {  // Cursor is set to start at index pos 0
                ...
                // position the cursor at index position Zero
                do {  // #1 columnDate assigned the cursor index position of databases colDate location
                    int columnDate = cursor.getColumnIndex(colDate);
                    // #2 then set the input date to the string value of that columnDate
                    currentItem.setInputDate(cursor.getString(columnDate));
                    // #1 ...
                    ...
                    currentItem.setTextCost(cursor.getString(columnCost));
                    currentItem.setTextLitres(cursor.getString(columnLitres));
                    try { // try catch is used as there is possibility of record reaching
                        // unrecorded row position, which would throw a NPE
                        // cursor move to the next row in the db ..ie next record
                        nextItem = new VehicleDetails();
                        cursor.moveToNext();
    ...
    // get cost per mile
    public ArrayList<VehicleDetails> Cost_Per_Mile_Second_Vehicle() {  // clear the calculated list
